...
for c in passenger_classes:
    ...
    fares_by_class[c] = fare_for_class
...
def null_counter(series):
    null_count = pd.isnull(series)
    null = series[null_count]
    return len(null)
    
    
column_null_count = titanic_survival.apply(null_counter)
